...
bool WavefrontSceneGraphFactory::addWavefront(const char* fileName, glm::mat4 matrix, AAssetManager* asset_manager) {
#else
	bool WavefrontSceneGraphFactory::addWavefront(const char* fileName, glm::mat4 matrix) {
#endif
		...
		for (size_t s = 0; s < shapes.size(); s++) {
			...
			for (size_t f = 0; f < shapes[s].mesh.indices.size() / 3; f++) {
				...
				if ((current_material_id < 0)
					...
					LOGE("Invalid material index: %i reverting to default material.", current_material_id);
				}
			...
			if(geom_node->vertex_data.size() == 0) {
				...
				LOGE("Warning, scene node %s does not containing geometry, ommiting.", shapes[s].name.c_str());
				continue;
			}
			...
			for(auto it = geom_node->vertex_data.begin(); it != geom_node->vertex_data.end(); ++it) {
				Vertex* vert = &*it;
	...
	Node* WavefrontSceneGraphFactory::build() {
		TransformNode* group = new TransformNode();
		assert(group);
		...
		for(std::vector<GeometryNode*>::iterator it = this->geometry_nodes.begin(); it != this->geometry_nodes.end(); ++it) {
			...
			trans_node->matrix = glm::translate(glm::mat4(1.0f), glm::vec3(geom_node->center[0], geom_node->center[1], geom_node->center[2]));
